...
void DKDFrame::onSystemTrayIconClicked(QSystemTrayIcon::ActivationReason reason)
{
  switch(reason)
  {
  //单击
...
void DKDFrame::AddSysTrayMenu()
{
    minimizeAction = new QAction(tr("在线"), this);
    connect(minimizeAction, SIGNAL(triggered()), this, SLOT(exeitProgram()));
    maximizeAction = new QAction(tr("隐身"), this);
    connect(maximizeAction, SIGNAL(triggered()), this, SLOT(exeitProgram()));
    restoreAction  = new QAction(tr("忙碌"), this);
    connect(restoreAction, SIGNAL(triggered()), this, SLOT(exeitProgram()));
...
void DKDFrame::AddSysTray()
{
    trayicon = new QSystemTrayIcon(this);
    QIcon icon("C://linux32.png");
    trayiconMenu = new QMenu(this);
    //trayiconMenu->addAction(this->action_quit);
    trayicon->setIcon(icon);
    trayicon->show();
    trayicon->setToolTip(tr("DKD"));
    trayicon->setContextMenu(trayiconMenu);
    trayicon->showMessage(tr(""), tr("DKD"), QSystemTrayIcon::Information, 5000);
